...
Standalone HTML tables, not React components. "Paper doesn't invert" applies
here too: Gmail auto-inverts light backgrounds in dark mode and ignores
`<meta name="color-scheme">`, so every template needs `bgcolor` attributes,
a `prefers-color-scheme: dark` override, and opaque backing behind the
signature PNG. Full checklist and the campaign log: `emails/README.md`.

// Mirrors emails/stamped/confirm.html — keep both in sync if the brand
// treatment there changes.

import { sendEmail } from "./send";

function escapeHtml(value: string): string {
  return value
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
}

export function renderConfirmEmail(data: { confirmUrl: string }): {
  subject: string;
  html: string;
} {
  const confirmUrl = escapeHtml(data.confirmUrl);

  const html = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="color-scheme" content="light">
<meta name="x-apple-disable-message-reformatting">
<title>Confirm your subscription — STAMPED</title>
</head>
<body style="margin:0; padding:0; background-color:#e6ddd0; -webkit-text-size-adjust:100%;">

<div style="display:none; max-height:0; overflow:hidden; opacity:0; mso-hide:all;">
  One click and the weekly edit is yours.
  &nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;
</div>

<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#e6ddd0;">
  <tr>
    <td align="center" style="padding:28px 12px;">

      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#f6f1e8; border:1px solid #c9d6dc;">
        <tr>
          <td style="padding:6px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #d8cdbb;">

              <!-- Masthead -->
              <tr>
                <td align="center" style="padding:40px 40px 0 40px; font-family:Georgia,'Times New Roman',serif;">
                  <span style="font-size:24px; color:#1b3154; letter-spacing:-0.3px;">BON V:</span>
                  <span style="font-size:17px; font-style:italic; color:#223e67;">&nbsp;A Travel Company</span>
                  <div style="margin:22px auto 0 auto; width:52px; height:2px; background-color:#b78b42; line-height:2px; font-size:0;">&nbsp;</div>
                </td>
              </tr>

              <!-- Body -->
              <tr>
                <td align="center" style="padding:26px 48px 8px 48px; font-family:Georgia,'Times New Roman',serif; color:#1b3154;">
                  <div style="font-family:Arial,Helvetica,sans-serif; font-size:11px; letter-spacing:3px; color:#223e67;">STAMPED &middot; THE WEEKLY EDIT</div>
                  <h1 style="margin:16px 0 0 0; font-size:30px; font-weight:normal; line-height:1.2; color:#1b3154;">One more step,<br>and you&rsquo;re on the list.</h1>
                  <p style="margin:18px 0 0 0; font-size:17px; line-height:1.7; color:#223e67;">
                    Confirm your address and the next letter &mdash; the best fares
                    I&rsquo;ve negotiated on the world&rsquo;s finest cruise lines &mdash; will
                    find you this Saturday.
                  </p>
                </td>
              </tr>

              <!-- CTA -->
              <tr>
                <td align="center" style="padding:26px 40px 8px 40px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" align="center">
                    <tr>
                      <td style="background-color:#223e67;">
                        <a href="${confirmUrl}" style="display:inline-block; padding:16px 40px; font-family:Arial,Helvetica,sans-serif; font-size:12px; letter-spacing:2px; color:#f6f1e8; text-decoration:none;">
                          CONFIRM MY SUBSCRIPTION
                        </a>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>

              <tr>
                <td align="center" style="padding:6px 48px 12px 48px; font-family:Georgia,serif;">
                  <p style="margin:0; font-size:14px; font-style:italic; line-height:1.6; color:#607d99;">
                    Didn&rsquo;t sign up? No harm done &mdash; ignore this and you won&rsquo;t
                    hear from us again.
                  </p>
                </td>
              </tr>

              <!-- Sign-off -->
              <tr>
                <td align="center" style="padding:8px 40px 8px 40px; font-family:Georgia,'Times New Roman',serif;">
                  <p style="margin:0; font-size:22px; font-style:italic; color:#223e67;">Jordan Yates</p>
                  <p style="margin:2px 0 0 0; font-family:Arial,Helvetica,sans-serif; font-size:10px; letter-spacing:2px; color:#8ea6b4;">LUXURY VOYAGE ADVISOR</p>
                </td>
              </tr>

              <!-- Footer -->
              <tr>
                <td style="padding:24px 40px 30px 40px;">
                  <div style="border-top:1px solid #c9d6dc; padding-top:18px; font-family:Arial,Helvetica,sans-serif; font-size:11px; line-height:1.7; color:#8ea6b4; text-align:center;">
                    BON V: A Travel Company &middot; in partnership with Luxury Cruise Connections<br>
                    2420 NE 186th St, Suite 300, Miami, FL 33180 &middot; Virtuoso member since 2011
                  </div>
                </td>
              </tr>

            </table>
          </td>
        </tr>
      </table>

    </td>
  </tr>
</table>
</body>
</html>`;

  return { subject: "Confirm your subscription — STAMPED", html };
}

// Fires the double opt-in confirmation email. Never throws — a delivery
// failure here should not turn a successful signup into an error for the
// client.
export async function notifyDispatchConfirmation(data: {
  email: string;
  confirmUrl: string;
}): Promise<void> {
  const { subject, html } = renderConfirmEmail({ confirmUrl: data.confirmUrl });
  await sendEmail({ to: data.email, subject, html });
}
